...
<body>
 								...
 								<table class="table table-hover text-nowrap" id="mainTable">
									...
									<tbody>
										<c:forEach var="i" begin="0" end="6">
											<tr id="${i+1 }" onclick="javascript:clickTd(this.id);">
												<td>${i+1 }</td>
												<td>John ${i+1 }번</td>
												<td>11-7-2014</td>
											...
											<tr>
												<td>
													...
													<button type="button" onclick="javascript:ajaxGo();" class="btn btn-primary" style="">ajax</button>
													<button type="button" id="modalButton" class="btn btn-info" data-toggle="modal" data-target="#modal-info">
														모달
													</button>
												</td>
												<td></td>
	...
	<script>
		...
		function ajaxGo(){
			...
			$.ajax({
	        	url:'<%=request.getContextPath()%>/common/ajaxGo?tdId='+temp,
	            type:'get',
	            success:function(success){
	            	for(var i=0;i<5;i++){
	    				document.getElementById("change_body").getElementsByTagName("tr")[i].getElementsByTagName("td")[0].innerHTML = document.getElementById(success[0]).getElementsByTagName("td")[i].innerHTML;
	    			}
	            	console.log('ajax로 입력성공');
